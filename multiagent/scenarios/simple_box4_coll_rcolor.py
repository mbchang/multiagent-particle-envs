...
import numpy as np
import time
from multiagent.core import World, Agent, NFAgent, Landmark, BoxWorld, SlipperyBoxWorld, CollideSlipperyBoxWorld, PushingBoxWorld
...
class Scenario(BaseScenario):
    def make_world(self, k=4):
        ...
        world.landmarks[-1].movable = False  # target is not movable
        world.landmarks[-1].collide = False  # target is not movable
    ...
    def reward(self, agent, world):
        dist2 = np.sum(np.square(agent.state.p_pos - world.landmarks[-1].state.p_pos))
        return -dist2
